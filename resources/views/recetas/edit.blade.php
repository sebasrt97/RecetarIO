    ...
    <div class="py-4">
        <div class="max-w-xl mx-auto px-4">
            <div class="bg-white dark:bg-gray-900 border border-gray-200 dark:border-gray-700 rounded p-4">
                <form id="receta-form" method="POST" action="{{ route('recetas.update', $receta) }}" enctype="multipart/form-data" class="space-y-4">
                    @csrf
                    @method('PUT')
                    ...
                    <template id="ingrediente-filas">
                        <div class="ingrediente-fila flex items-center space-x-2 mb-2 p-2 border border-gray-300 dark:border-gray-700 rounded">
                            <select name="ingredientes[INDEX][id]" required
                                ...
                                @foreach($ingredientesDisponibles as $disponible)
                            ...
                            <input type="text" name="ingredientes[INDEX][unidad_receta_medida]" placeholder="Unidad (ej: gr, ml, unidad)" required
                                   class="w-1/4 border border-gray-300 dark:border-gray-600 px-2 py-1 rounded text-sm dark:bg-gray-800 dark:text-white">
 ...
 {{-- Script para la lógica de ingredientes dinámicos --}}
    <script>
        document.addEventListener('DOMContentLoaded', function () {
            ...
            let ingrediente = 0; // Para asignar índices únicos a los campos de nombre
            ...
            function añadirIngrediente(ingredienteDato = null) {
                ...
                if (ingredienteDato) {
                    ...
                    // Asegurarse de seleccionar el ingrediente correcto
                    if (selectId) selectId.value = ingredienteDato.ingrediente_id || ingredienteDato.id || ''; // 'ingrediente_id' para existing, 'id' for old()
            ...
            // Lógica para cargar ingredientes existentes o 'old' values
            const ingredienteAntiguo = @json(old('ingredientes', []));
            const ingredienteExiste = @json($receta->ingredientes->map(function($relacion) {
                return [
                    'ingrediente_id' => $relacion->id,
                    'cantidad_bruta' => $relacion->pivot->cantidad_bruta,
                    'unidad_receta_medida' => $relacion->pivot->unidad_receta_medida,
                ];
            }));

            let ingredienteArray = [];
            if (ingredienteAntiguo.length > 0) {
                // Si hay 'old' values (errores de validación), úsalos
                ingredienteArray = ingredienteAntiguo;
            } else if (ingredienteExiste.length > 0) {
                // Si no hay 'old' values, usa los ingredientes de la receta
            ...
            if (ingredienteArray.length > 0) {
                ...
                // Si no hay ninguno, añadir una fila vacía para empezar
                añadirIngrediente();
            }

            // Listener para el botón "Añadir Ingrediente"
